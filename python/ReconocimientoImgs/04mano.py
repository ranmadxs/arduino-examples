...
while(1):
    ...
    thh = th.copy()
    imageCountours, cnt, hie = cv2.findContours(th,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    cov = len(cnt)
    numb = np.array([])
    if(cov>0):
        for i in range(cov):
